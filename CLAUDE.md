...
| `07-paper/` | `paper-summary` | 论文总结集（`paper_summaries.md` 单文件持续追加，每篇二级标题分隔） |
| `08-book/` | `book` | 书籍资料整理/读书笔记 |
...
- **维护页箭头字符**：`INDEX.md`/`_index.md` 等维护页箭头统一用 Unicode `->`（U+2192）、`←`（U+2190），**非 ASCII `->`/`<-`**；Edit 时从原文复制字符，新增行对齐，否则字形不齐且 old_string 易因码点不同而匹配失败。
